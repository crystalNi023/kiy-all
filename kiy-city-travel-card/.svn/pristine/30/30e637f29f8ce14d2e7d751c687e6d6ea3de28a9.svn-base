...
//	private static final String SELECT_USER = "SELECT u.*, count(w.`user`) cardsNum from users u ,write_card_record w where u.id=w.`user` GROUP BY u.id";
	private static final String SELECT_USER = "SELECT u.*,IFNULL(b.count,0) cardsNum from users u LEFT JOIN (select w.`user` user,COUNT(w.`user`) count FROM write_card_record w GROUP BY w.`user`) b on u.id = b.`user`";
	...
	public static List<User> selectUser() {
		...
		try {
			...
			while (rs.next()) {
				...
				user.setEnable(rs.getBoolean("enable"));
				user.setCardsNum(rs.getInt("cardsNum"));
