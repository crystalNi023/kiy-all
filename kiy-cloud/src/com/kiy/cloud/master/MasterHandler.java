...
import com.kiy.cloud.http.CloudResponseListener;
import com.kiy.cloud.push.PushUtil;
...
public final class MasterHandler extends SimpleChannelInboundHandler<Messages.Message> {
	...
	public static final String CLOUD = "cloud-"; //云端管理端请求标志

	
	private static List<CloudResponseListener> listeners = new ArrayList<CloudResponseListener>();
	 ...
	 * @param listener
	 */
	public static void addCloudResponseListener(CloudResponseListener listener) {
	 ...
	 * @param listener
	 */
	public static void removeCloudResponseListener(CloudResponseListener listener) {
...
//					}
					...
					connectBuilder.setRemotePort(s.getRemote_port());
...
//			case DELETE_USER:
				...
				if (client.isChecked()) {
					switch (client.getType()) {
						...
						case Client.SMART_HOME_SERVO:
							...
							if (msg.getUserId()!=null&&msg.getUserId().contains(CLOUD)) {
								for (CloudResponseListener cloudResponseListener : listeners) {
									cloudResponseListener.receive(msg);
								}
							}else {
								// 仅回复给指定用户 1
			...
			/**
			 * 8.控制台手机端都可以访问,返回给指定的服务器（未传userId）
			 */
			case URGENT_MESSAGE:
			case MAIN_ACCOUNT:
			...
			case OFFLINE_NOTICE_RECEIVE:{
				group.SendClients(msg, client.getServo());/*收到本地服务器发送的指令 返回给所有用户（主动发出的下线通知）*/
				break;
			}
			case NOTICE_CLOUD:{
				NoticeCloud noticeCloud = msg.getNoticeCloud();
				String deviceToken = noticeCloud.getDeviceToken();
				String noticeContent = noticeCloud.getNoticeContent();
				if (deviceToken.isEmpty()||noticeContent.isEmpty()) {
					break;
				}
				if(noticeCloud.getType()==1){//Android
					System.err.println("发送设备类型:"+deviceToken+"通知内容："+noticeContent);
					if(PushUtil.sendAndroidUnicast(deviceToken,noticeContent)){
						System.err.println("发送通知成功");
					}else{
						Message.Builder mb = msg.toBuilder();
						mb.setResult(Result.ERROR);
						ctx.channel().writeAndFlush(mb.build());
					}
					break;
				}else if(noticeCloud.getType()==2){//Ios
					System.err.println("发送设备类型:"+deviceToken+"通知内容："+noticeContent);
					if(PushUtil.sendIOSUnicast(deviceToken,noticeContent)){
						System.err.println("发送通知成功");
					}else{
						Message.Builder mb = msg.toBuilder();
						mb.setResult(Result.ERROR);
	...
	public static void main(String[] args) {
		try {
			PushUtil.sendIOSUnicast("82229786c07ac2ce9900958d191f580c1b13fae59627704f720f32372b147243","xixixi");
		} catch (Exception e) {
