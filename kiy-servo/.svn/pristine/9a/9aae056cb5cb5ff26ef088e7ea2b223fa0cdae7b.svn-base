 ...
 *
 */
public final class MudbusSerialPort extends SerialPortReceiver {
	...
	public MudbusSerialPort(Device d) {
		...
		getSTATUS_0x01.add(Model.MB_0020);
		getSTATUS_0x01.add(Model.MB_0020_01);
		
		getSTATUS_0x02.add(Model.MB_0007);
		getSTATUS_0x02.add(Model.MB_0011);
		...
		setSTATUS_0x05.add(Model.MB_0020);
		setSTATUS_0x05.add(Model.MB_0020_01);
	...
	public synchronized void getStatus(Device d) {
		...
		if (System.currentTimeMillis() - d.getTickStatus() > Config.DRIVER_INTERVAL * 1000) {
			...
			if (getSTATUS_0x01.contains(model)) {
				...
				return;
			} else if (model == Model.MB_0021 || model == Model.MB_0022) {
				// 飞音云根据通讯适配器是否在线判断在线与否
				d.setStatus(isActive() ? Status.NONE : Status.OFFLINE);
			} else {
