...
public abstract class SerialPortReceiver extends DriverAdpater {
	...
	public void start() {
		...
		try {
			cp = portIdentifier.open(SerialPortReceiver.class.getName(), 2000);
	...
	public void receive() {
		...
		if (Config.DEBUG)
			Log.debug("SerialPortReceiver-001");
		// DECODE
	...
	public synchronized void send(Device device, byte code, int tag) {
		...
		// ENCODE
		if (Config.DEBUG)
			Log.debug("SerialPortReceiver-002");
		...
		if (Config.DEBUG)
		Log.debug("SerialPortReceiver-003");
		// DECODE
