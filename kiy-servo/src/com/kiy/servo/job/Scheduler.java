...
public final class Scheduler implements Runnable {
	...
	private boolean notice = true; // 默认是发送通知（用于五分钟后不再发送通知）
...
//						rd.setStatus(Status.THRESHOLD);
						if(rd.getNotice()) {
							...
							if (lastPush == 0 ) {
								...
								notice=true;
							}else if(notice){/*第二次发送间隔五分钟（报警只推送两次）*/
								long currentTimeMillis = System.currentTimeMillis();
								if(currentTimeMillis-task.getLastPush()>=300000){
									sendThreshold(rd,true);
									notice = false;//第二次推送后不再发出通知
									task.setLastPush(System.currentTimeMillis());
								}else {
									// 距离上一次推送 小于10秒钟 不推送
								}
					...
					}else if(feature.getValue()>=task.getLimitUpper()) {
						upper = true;
//						rd.setStatus(Status.THRESHOLD);
						if(rd.getNotice()) {
							long lastPush = task.getLastPush(); /*上一次推送时间*/
	...
	private void sendThreshold(Device d,boolean isLower) {
		...
		case SERIAL_RADIO_FREQUENCY:
			sb.append(d.getName()+"报警，请及时检查并处理。");
			break;
		case SENSOR_POWER:
			sb.append(d.getName()+"报警，请及时检查并处理。");
			break;
